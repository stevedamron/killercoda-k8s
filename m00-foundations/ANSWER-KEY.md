...
- **Step 3 (kubectl idioms):** A fluency drill — the day-to-day verbs (`get / describe / logs / exec / apply / edit / delete / port-forward`), the flags you combine constantly (`-n -A -l --watch -f -c --previous -o wide/yaml/json`), how `kubectl edit` differs from `kubectl apply`, and the two self-orientation commands (`kubectl api-resources`, `kubectl explain`) for landing on unfamiliar clusters. None of it is broken; you're building the muscle memory.
