...
#     learner has to climb the owner chain (`describe rs` / `get events`).
...
spec:
  hard: { pods: "2" }   # MUTATED (baseline: pods: "5")
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
spec:
  replicas: 3   # MUTATED (baseline: replicas: 2) — exceeds quota by 1
...
# Wait for the healthy fleet to come up. port-processor will never become
# Available (only 2 of 3 desired pods can exist due to quota); don't block on it.
